...
bool U, V, P, Q; 
   ...
   //IO Pins
    ...
    PyHal_GPIO_SetDir(21,1); //Output
    PyHal_GPIO_SetDir(18,1); //Output
    
    while(1) //If both Green and Blue LED turn on LHS=RHS
    {
    	U = PyHal_GPIO_Get(2);
    	V = PyHal_GPIO_Get(4);
    	P = !U|V;
    	PyHal_GPIO_Set(21,P); //If condition is true, Green LED turns on
    	Q =(!U&!V)|(!U&V)|(U&V);
    	PyHal_GPIO_Set(18,Q); //If condition is true, Blue LED turns on
    } 
